...
data = sqlite3.connect("data_ML.db")
...
# c.execute("UPDATE laptop SET Brand = 'MacBook Pro' WHERE Brand = '...'")

# c.execute("DELETE FROM laptop WHERE CPU LIKE '%i3%'")

def Show_mobile_phones_before_2020(c):
    for i in c.execute("SELECT * FROM mobile WHERE year < 2020"):
        print(f"{i[0]}, {i[1]}, {i[5]}")
    
def Display_of_laptops_and_mobile_above_5000_euros(c):
...
while True:
    op = int(input("1= Show_mobile_phones_before_2020, 2= Display_of_laptops_and_mobile_above_5000_euros, 3= Show_5_laptops_with_the_most_amount_of_ssd, 4= Exit and save\n"))
    if op == 1:
        Show_mobile_phones_before_2020(c)
    elif op == 2:
        Display_of_laptops_and_mobile_above_5000_euros(c)
    elif op == 3:
